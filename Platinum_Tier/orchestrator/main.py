"""
Orchestrator - Main controller for the Gold Tier AI Employee.

This is the primary entry point. The orchestrator:
1. Starts all Watchers (Gmail, WhatsApp, LinkedIn, FileSystem, Facebook)
...
5. Triggers Claude Code to process items (with Ralph Wiggum loop for autonomy)
6. Generates Weekly CEO Briefing (Monday 10 AM)
7. Integrates with Odoo Accounting and Facebook (Gold Tier)

Gold Tier: Monitors categorized subdirectories in /Needs_Action/ with Odoo + Facebook integration
           Includes Ralph Wiggum loop for autonomous multi-step task completion
...
# Import Ralph Wiggum for autonomous task completion
from ralph_wiggum import get_task_tracker, TaskState, StopHook
...
class ColoredFormatter(logging.Formatter):
    ...
    COMPONENT_COLORS = {
        ...
        'ODOO': Fore.MAGENTA,  # Purple for Odoo accounting
        'FACEBOOK': Fore.CYAN,  # Cyan for Facebook
    }
...
watcher_names = ['FILES', 'GMAIL', 'WHATSAPP', 'LINKEDIN', 'FACEBOOK']
...
VALID_CATEGORIES = ['email', 'whatsapp', 'linkedin', 'files', 'odoo', 'facebook']


class Orchestrator:
    """
    Main orchestrator for Gold Tier AI Employee.

    Coordinates:
    - All Watchers (Gmail, WhatsApp, LinkedIn, FileSystem, Facebook)
    ...
    def __init__(self, vault_path: str, check_interval: int = 180):
        ...
        Args:
            ...
            check_interval: Seconds between checks for new items (default: 180)
        """
        import os
        
        # ==========================================
        # PLATINUM TIER - Execution Mode Support
        # ==========================================
        ...
        logger.info(f"PLATINUM TIER - Execution Mode: {self.execution_mode}")
        logger.info(f"  Cloud Agent: {self.is_cloud_agent}")
        if self.is_cloud_agent:
            logger.info(f"  ⚠️  DRAFT-ONLY MODE - No real sending/posting")
        # ==========================================
        ...
        # PLATINUM TIER - Updates folder for Cloud→Local handoff
        ...
        logger.info(f"Orchestrator initialized (Gold Tier)")
    ...
    # ==========================================
    # PLATINUM TIER - Write Update Method
    # ==========================================
    def write_update(self, update_type: str, data: dict):
        """
        Write update to /Updates/ folder (Cloud Agent only).
        This is how Cloud Agent communicates with Local Agent.
        
        Args:
            update_type: Type of update (e.g., 'email_draft', 'linkedin_draft')
        ...
        update_file = self.updates / f"{update_type}_{int(time.time())}.json"
        update_content = {
            "type": update_type,
        ...
        try:
            ...
            logger.info(f"[PLATINUM] Wrote update to {update_file.name}")
    ...
    def claim_file(self, filepath: Path) -> bool:
        ...
        Prevents double-processing by Cloud and Local agents.
        
        Args:
            filepath: Path to file to claim
            
        Returns:
            True if successfully claimed, False if already claimed
        """
        agent_name = "cloud" if self.is_cloud_agent else "local"
        ...
        if dest.exists():
            logger.warning(f"File {filepath.name} already claimed by {agent_name}")
            return False
        
        try:
            shutil.move(str(filepath), str(dest))
            logger.info(f"[PLATINUM] {agent_name.upper()} claimed {filepath.name}")
            return True
        except Exception as e:
            logger.error(f"Failed to claim {filepath.name}: {e}")
            return False
    
    def is_file_claimed(self, filepath: Path) -> tuple:
        """
        Check if file is claimed and by which agent.
        
        Returns:
            (is_claimed, claiming_agent)
        """
        for agent in ['cloud', 'local']:
            agent_folder = self.in_progress / agent
            if (agent_folder / filepath.name).exists():
                return True, agent
        return False, None
    # ==========================================
    ...
    def start_watcher(self, watcher_type: str):
        ...
        Args:
            watcher_type: Type of watcher ('filesystem', 'gmail', 'whatsapp', 'linkedin', 'facebook')
        """
        import subprocess
        import sys
        import os
    ...
    def start_all_watchers(self):
        """Start all available watchers (Gold Tier: Gmail, WhatsApp, LinkedIn, FileSystem, Facebook)."""
        logger.info("Starting all watchers (Gold Tier)...")
        self.start_watcher('filesystem')
        self.start_watcher('gmail')
        self.start_watcher('whatsapp')
        self.start_watcher('linkedin')
        self.start_watcher('facebook')  # Facebook watcher for social media monitoring
        logger.info("All Gold Tier watchers started")
    ...
    def _check_ralph_completion(self, initial_items: list) -> bool:
        """
        Check if Ralph Wiggum task is complete by verifying files moved to /Done/.

        Args:
            initial_items: List of initially pending items

        Returns:
            True if all items processed (moved to Done or Pending_Approval)
        """
        for item in initial_items:
            # Check if file was moved to Done
            done_path = self.done / item.parent.name / item.name
            if done_path.exists():
                continue
            
            # Check if file was moved to Pending_Approval (approval request created)
            # The original action file should be in In_Progress, approval in Pending_Approval
            in_progress_path = self.in_progress / item.parent.name / item.name
            pending_approval_files = list(self.pending_approval.glob(f"*{item.stem}*"))
            
            if in_progress_path.exists() and not pending_approval_files:
                # File still in progress, not completed
                return False
            
            # File is either done or waiting for approval
            continue
    ...
    def trigger_claude_process(self, items_by_category: dict) -> bool:
        ...
        Uses Ralph Wiggum loop for autonomous multi-step task completion.

        Ralph Wiggum Loop:
        1. Run Claude with prompt
        2. Check if files moved to /Done/ or /Pending_Approval/
        3. If incomplete and under max iterations, re-run with updated prompt
        ...
        # Initialize Ralph Wiggum task tracker
        tracker = get_task_tracker(str(self.vault_path))
        task_name = f"process_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        max_iterations = 5  # Max Ralph iterations
        tracker.start_task(task_name, max_iterations=max_iterations)

        logger.info(f"Starting Ralph Wiggum Loop: Processing {len(all_items)} items (max {max_iterations} iterations)")

        # Ralph Wiggum Loop: Keep running Claude until task complete or max iterations
        while tracker.should_continue():
            iteration = tracker.get_iteration()
            logger.info(f"{'='*60}")
            logger.info(f"RALPH WIGGUM ITERATION {iteration}/{max_iterations}")
            logger.info(f"{'='*60}")

            # Build category summary (only for remaining items)
            remaining_items = []
            category_summary = []
            for category, cat_items in items_by_category.items():
                remaining = [item for item in cat_items if item.name not in self.processed_files]
                if remaining:
                    remaining_items.extend(remaining)
                    category_summary.append(f"  /Needs_Action/{category}/: {len(remaining)} files")
...
- [PENDING] Created approval request: filename

CRITICAL: ALL file operations MUST use category subfolders:
- Move to /In_Progress/<category>/ (e.g., /In_Progress/email/file.md)
- Create Plan.md in /Plans/<category>/ (e.g., /Plans/email/PLAN_xxx.md)
- Move to /Done/<category>/ (e.g., /Done/email/file.md)
- Create approval in /Pending_Approval/<category>/
...
3. For each file in /Needs_Action/<category>/:
   a. [MOVE] Move to /In_Progress/<category>/ (claim ownership)
   b. [WRITE] Create Plan.md in /Plans/<category>/ folder
   c. [READ] Read original file from /Archive/<category>/ if needed
   d. Determine if sensitive:
      - ROUTINE: Execute action, move to /Done/<category>/
...
COMPLETION: When all files are processed and moved to /Done/<category>/, output:
<promise>TASK_COMPLETE</promise>"""

            # Run Claude
            result = self._run_claude(prompt, remaining_items, self.processed_files)

            # Mark processed files
            for item in remaining_items:
                tracker.mark_file_processed(item.name, "processed")

            # Check completion
            if result and self._check_ralph_completion(remaining_items):
                logger.info("Task completion verified - all files processed")
                tracker.mark_task_complete()
                break
            elif result:
                logger.info("Claude completed but task may not be finished - checking...")
                # Continue to next iteration if files still need processing
            else:
                logger.warning(f"Claude failed on iteration {iteration}")
                tracker.mark_error(f"Claude failed on iteration {iteration}")

            # Prepare for next iteration
            if tracker.should_continue():
                logger.info(f"Task not complete, preparing iteration {iteration + 1}...")
                tracker.increment_iteration()
                import time
                time.sleep(2)  # Brief pause before next iteration
            else:
                logger.warning(f"Max iterations ({max_iterations}) reached")
                tracker.mark_task_failed("Max iterations reached")
                break

        # Final summary
        summary = tracker.get_summary()
        logger.info(f"{'='*60}")
        logger.info(f"RALPH WIGGUM LOOP FINISHED")
        logger.info(f"{'='*60}")
        logger.info(f"Final state: {summary['state']}")
        logger.info(f"Iterations used: {summary['iteration']}")
    ...
    def execute_approved_directly(self, items: list) -> bool:
        ...
        for item in items:
            try:
                ...
                if action_type == 'email':
                ...
                elif action_type == 'odoo':
                    # Extract Odoo action details from approval request
                    # Check if it's an invoice creation or payment recording
                    if 'create_invoice' in approval_content.lower() or 'invoice' in approval_content.lower():
                        # Extract invoice details
                        partner_match = re.search(r'(?:Partner|Customer|To):\s*([^\n]+)', approval_content)
                        amount_match = re.search(r'(?:Amount|Total|Price):\s*\$?([\d,]+\.?\d*)', approval_content)
                        desc_match = re.search(r'(?:Description|For|Service):\s*([^\n]+)', approval_content)

                        if partner_match and amount_match:
                            partner = partner_match.group(1).strip()
                            amount = float(amount_match.group(1).replace(',', ''))
                            description = desc_match.group(1).strip() if desc_match else "Invoice"

                            logger.info(f"[ODOO] Creating invoice for {partner}: ${amount}")

                            try:
                                from mcp_server.odoo_service import OdooService
                                import asyncio

                                odoo_service = OdooService(str(self.vault_path))

                                if odoo_service._connect():
                                    result = asyncio.run(odoo_service.create_invoice(
                                    ...
                                    if result.get('success'):
                                        logger.info(f"[ODOO] ✓ Invoice created: {result.get('invoice_id', 'N/A')}")
                                        ...
                                        logger.error(f"[ODOO] ✗ Failed to create invoice: {result.get('error', 'Unknown error')}")
                                else:
                                    logger.error(f"[ODOO] ✗ Failed to connect to Odoo")
                            except Exception as e:
                                logger.error(f"[ODOO] ✗ Error creating invoice: {str(e)}")

                    elif 'record_payment' in approval_content.lower() or 'payment' in approval_content.lower():
                        # Extract payment details
                        invoice_match = re.search(r'(?:Invoice|Invoice ID|Invoice #):\s*([^\n]+)', approval_content)
                        amount_match = re.search(r'(?:Amount|Payment):\s*\$?([\d,]+\.?\d*)', approval_content)

                        if invoice_match and amount_match:
                            invoice_id = invoice_match.group(1).strip()
                            amount = float(amount_match.group(1).replace(',', ''))

                            logger.info(f"[ODOO] Recording payment ${amount} for invoice {invoice_id}")
                            ...
                            try:
                                ...
                                if odoo_service._connect():
                                    result = asyncio.run(odoo_service.record_payment(
                                        invoice_id=int(invoice_id) if invoice_id.isdigit() else 1,
                                        amount=amount
                                    ))

                                    if result.get('success'):
                                        logger.info(f"[ODOO] ✓ Payment recorded: {result.get('payment_id', 'N/A')}")
                                        ...
                                        logger.error(f"[ODOO] ✗ Failed to record payment: {result.get('error', 'Unknown error')}")
                                else:
                                    logger.error(f"[ODOO] ✗ Failed to connect to Odoo")
                            except Exception as e:
                                logger.error(f"[ODOO] ✗ Error recording payment: {str(e)}")

                elif action_type == 'facebook':
                    # Extract Facebook post details from approval request
                    content_match = re.search(r'\*\*Content:\*\*([\s\S]*?)(?=\n---|\n##|\Z)', approval_content)
                    ...
                    if content_match:
                        ...
                        logger.info(f"[FACEBOOK] Posting to Facebook (Length: {len(post_text)} chars)")
                        ...
                        try:
                            ...
                            result = asyncio.run(facebook_service.post_facebook(message=post_text))

                            if result.get('success'):
                                logger.info(f"[FACEBOOK] ✓ Post created successfully (Post ID: {result.get('post_id', 'N/A')})")
                                ...
                                logger.error(f"[FACEBOOK] ✗ Failed to post: {result.get('error', 'Unknown error')}")
                        except Exception as e:
                            logger.error(f"[FACEBOOK] ✗ Error posting to Facebook: {str(e)}")
                    else:
                        logger.warning(f"[FACEBOOK] Could not extract post content from approval request")
    ...
    def _get_odoo_financial_data(self, period: str = 'week') -> dict:
        """Fetch financial data from Odoo."""
        try:
            from mcp_server.odoo_service import OdooService
            import asyncio

            service = OdooService(vault_path=str(self.vault_path))
            if not service._connect():
                return {'success': False, 'error': 'Connection failed'}

            result = asyncio.run(service.get_financial_report(period=period))
            unpaid = asyncio.run(service.list_unpaid_invoices())
            
            return {
                'success': True,
                'revenue': result.get('report', {}).get('revenue', 0),
                'expenses': result.get('report', {}).get('expenses', 0),
                'profit': result.get('report', {}).get('profit', 0),
                'invoice_count': result.get('report', {}).get('invoice_count', 0),
                'unpaid_invoices': unpaid.get('invoices', []),
                'unpaid_count': unpaid.get('count', 0)
            }
        except Exception as e:
    ...
    def _get_facebook_metrics(self) -> dict:
        ...
        try:
            ...
            posts_result = asyncio.run(service.get_posts(limit=10))
            page_result = asyncio.run(service.get_page_info())
    ...
    def _get_all_vault_statistics(self) -> dict:
        """
        Get comprehensive vault statistics from ALL sources.
        
        Includes: Email, WhatsApp, LinkedIn, Facebook, Files, Odoo
        """
        stats = {
            'folders': {},
            'by_category': {},
        ...
        for name, folder in folders.items():
            if folder.exists():
                ...
                stats['folders'][name] = count

        # Count by category (ALL sources)
        categories = ['email', 'whatsapp', 'linkedin', 'files', 'facebook', 'odoo']
        for category in categories:
            # Needs_Action count
            cat_path = self.needs_action / category
            if cat_path.exists():
                count = len(list(cat_path.glob('*.md')))
                stats['by_category'][f'{category}_pending'] = count
            
            # Done count
            done_path = self.done / category
            if done_path.exists():
                done_count = len(list(done_path.glob('*.md')))
                stats['by_category'][f'{category}_done'] = done_count
            
            # Archive count
            archive_path = self.archive / category
            if archive_path.exists():
                archive_count = len(list(archive_path.glob('*.md')))
                stats['by_category'][f'{category}_archived'] = archive_count

        # Get recent activity from all categories
        all_done_files = []
        for category in categories:
            done_path = self.done / category
            if done_path.exists():
        ...
        # Sort by modification time and take last 15
        all_done_files.sort(key=lambda x: x['modified'], reverse=True)
        stats['recent_activity'] = all_done_files[:15]

        # Count completed today and this week
        now = datetime.now()
        for f in all_done_files:
            age_hours = (now - f['modified']).total_seconds() / 3600
            if age_hours < 24:
    ...
    def generate_briefing(self):
        """
        Generate Weekly CEO Briefing with ALL data sources.
        
        Gold Tier: Integrates data from:
        - Odoo (Accounting/Financials)
        ...
        """
        logger.info("Generating Weekly CEO Briefing (Gold Tier - All Sources)...")

        self.briefings.mkdir(parents=True, exist_ok=True)

        # Fetch ALL data sources
        logger.info("Fetching Odoo financial data...")
        odoo_data = self._get_odoo_financial_data(period='week')

        logger.info("Fetching Facebook metrics...")
        facebook_data = self._get_facebook_metrics()

        logger.info("Gathering vault statistics (all categories)...")
        vault_stats = self._get_all_vault_statistics()

        # Read Business_Goals.md
        business_goals = ""
        business_goals_path = self.vault_path / 'Business_Goals.md'
        if business_goals_path.exists():
            business_goals = business_goals_path.read_text(encoding='utf-8', errors='replace')

        # Calculate date for filename
        today = datetime.now()
        days_since_monday = today.weekday()
        monday = today - timedelta(days=days_since_monday)
        briefing_file = self.briefings / f"{monday.strftime('%Y-%m-%d')}_Monday_Briefing.md"

        # Build comprehensive briefing
        briefing = f"""---
generated: {datetime.now().isoformat()}
period: {(monday - timedelta(days=7)).strftime('%Y-%m-%d')} to {monday.strftime('%Y-%m-%d')}
...
"""
        ...
        if odoo_data.get('success'):
            briefing += f"""| Metric | Amount |
|--------|--------|
| Revenue This Week | ${odoo_data['revenue']:,.2f} |
| Expenses This Week | ${odoo_data['expenses']:,.2f} |
| **Profit This Week** | **${odoo_data['profit']:,.2f}** |
| Invoices Processed | {odoo_data['invoice_count']} |
| Unpaid Invoices | {odoo_data['unpaid_count']} |

"""
        else:
            briefing += f"*Odoo data unavailable: {odoo_data.get('error', 'Unknown error')}*\n\n"
...
"""
        if facebook_data.get('success'):
            briefing += f"""- **Posts This Week:** {facebook_data['post_count']}
- **Page:** {facebook_data.get('page_info', {}).get('name', 'N/A')}

"""
        else:
            briefing += f"*Facebook data unavailable*\n\n"

        # LinkedIn stats
        linkedin_done = vault_stats['by_category'].get('linkedin_done', 0)
        linkedin_pending = vault_stats['by_category'].get('linkedin_pending', 0)
        briefing += f"""### LinkedIn
- **Completed Actions:** {linkedin_done}
- **Pending Items:** {linkedin_pending}
...
### Email (Gmail)
- **Pending:** {vault_stats['by_category'].get('email_pending', 0)}
- **Completed:** {vault_stats['by_category'].get('email_done', 0)}
- **Archived:** {vault_stats['by_category'].get('email_archived', 0)}

### WhatsApp
- **Pending:** {vault_stats['by_category'].get('whatsapp_pending', 0)}
- **Completed:** {vault_stats['by_category'].get('whatsapp_done', 0)}
- **Archived:** {vault_stats['by_category'].get('whatsapp_archived', 0)}
...
### Files
- **Pending:** {vault_stats['by_category'].get('files_pending', 0)}
- **Completed:** {vault_stats['by_category'].get('files_done', 0)}
- **Archived:** {vault_stats['by_category'].get('files_archived', 0)}
...
| Metric | Count |
|--------|-------|
| **Total Completed (All Time)** | {vault_stats['folders'].get('Done', 0)} |
| **Completed Today** | {vault_stats['completed_today']} |
| **Completed This Week** | {vault_stats['completed_this_week']} |
| **Pending Approvals** | {vault_stats['folders'].get('Pending_Approval', 0)} |
| **In Progress** | {vault_stats['folders'].get('In_Progress', 0)} |
| **Awaiting Action** | {vault_stats['folders'].get('Needs_Action', 0)} |
...
|----------|---------|-----------|----------|
| Email | {vault_stats['by_category'].get('email_pending', 0)} | {vault_stats['by_category'].get('email_done', 0)} | {vault_stats['by_category'].get('email_archived', 0)} |
| WhatsApp | {vault_stats['by_category'].get('whatsapp_pending', 0)} | {vault_stats['by_category'].get('whatsapp_done', 0)} | {vault_stats['by_category'].get('whatsapp_archived', 0)} |
| LinkedIn | {vault_stats['by_category'].get('linkedin_pending', 0)} | {vault_stats['by_category'].get('linkedin_done', 0)} | {vault_stats['by_category'].get('linkedin_archived', 0)} |
| Facebook | {vault_stats['by_category'].get('facebook_pending', 0)} | {vault_stats['by_category'].get('facebook_done', 0)} | {vault_stats['by_category'].get('facebook_archived', 0)} |
| Files | {vault_stats['by_category'].get('files_pending', 0)} | {vault_stats['by_category'].get('files_done', 0)} | {vault_stats['by_category'].get('files_archived', 0)} |
| Odoo | {vault_stats['by_category'].get('odoo_pending', 0)} | {vault_stats['by_category'].get('odoo_done', 0)} | {vault_stats['by_category'].get('odoo_archived', 0)} |
...
|-----------|----------|------|
"""
        for activity in vault_stats['recent_activity']:
            timestamp = activity['modified'].strftime('%Y-%m-%d %H:%M')
            briefing += f"| {timestamp} | {activity['category']} | {activity['name']} |\n"
...
"""
        suggestions = []
        if vault_stats['by_category'].get('email_pending', 0) > 3:
            suggestions.append(f"- **Email**: {vault_stats['by_category']['email_pending']} emails need responses")
        if vault_stats['by_category'].get('whatsapp_pending', 0) > 2:
            suggestions.append(f"- **WhatsApp**: {vault_stats['by_category']['whatsapp_pending']} messages need replies")
        if vault_stats['folders'].get('Pending_Approval', 0) > 0:
            suggestions.append(f"- **Approvals**: {vault_stats['folders']['Pending_Approval']} items awaiting your review")
        if odoo_data.get('success') and odoo_data['unpaid_count'] > 0:
            suggestions.append(f"- **Accounting**: Follow up on {odoo_data['unpaid_count']} unpaid invoices")
        if vault_stats['by_category'].get('facebook_pending', 0) > 0:
            suggestions.append(f"- **Facebook**: {vault_stats['by_category']['facebook_pending']} items need attention")
        if vault_stats['by_category'].get('linkedin_pending', 0) > 0:
            suggestions.append(f"- **LinkedIn**: {vault_stats['by_category']['linkedin_pending']} items need attention")
        if vault_stats['by_category'].get('files_pending', 0) > 0:
            suggestions.append(f"- **Files**: {vault_stats['by_category']['files_pending']} files need processing")
        
        if suggestions:
            for s in suggestions:
                briefing += f"{s}\n"
        else:
            briefing += "All systems clear! No urgent items need attention.\n"
...
"""
        if business_goals:
            if 'Revenue Target' in business_goals:
                lines = business_goals.split('\n')
                for line in lines[:10]:
                    if any(kw in line for kw in ['Revenue', 'Monthly', 'Current', 'Target', 'Goal']):
                        briefing += f"{line}\n"
        else:
            briefing += "*Business_Goals.md not found - consider creating one*\n"
...
"""
        ...
        logger.info("=" * 60)
        logger.info(f"  Completed Today: {vault_stats['completed_today']}")
        logger.info(f"  Completed This Week: {vault_stats['completed_this_week']}")
        logger.info(f"  Pending Items: {vault_stats['folders'].get('Needs_Action', 0)}")
        if odoo_data.get('success'):
            logger.info(f"  Revenue: ${odoo_data['revenue']:,.2f}")
            logger.info(f"  Profit: ${odoo_data['profit']:,.2f}")
        logger.info("=" * 60)
...
def show_status(vault_path: str):
    ...
    print("\n" + "=" * 50)
    print("Personal AI Employee - Gold Tier Status")
    print("=" * 50)
...
The orchestrator monitors:
    - /Needs_Action/{email,whatsapp,linkedin,files,odoo,facebook}/: New tasks by category
