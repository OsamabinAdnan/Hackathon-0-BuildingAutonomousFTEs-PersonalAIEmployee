...
def run_facebook_watcher(vault_path: str):
    ...
    try:
        ...
        logger.error(f'Facebook watcher not available: {e}')
...
Available Watchers:
    ...
    facebook    Facebook Page watcher (monitors messages/comments)
...
Examples:
    ...
    python -m watchers --watcher linkedin   # LinkedIn only
    python -m watchers --watcher facebook   # Facebook only
        """
    )
    parser.add_argument(
        '--vault-path',
    ...
    parser.add_argument(
        ...
        type=str,
        default='all',
        choices=['all', 'file', 'gmail', 'whatsapp', 'linkedin', 'facebook'],
