...
1. Creates an action file in /Needs_Action/files/
2. Moves the original file to /Archive (to prevent re-detection)

Silver Tier: Writes to categorized /Needs_Action/files/ subdirectory.

Usage:
    python -m watchers --vault-path ./AI_Employee_Vault_FTE
    python -m watchers.filesystem
"""
...
class InboxHandler(FileSystemEventHandler):
    ...
    def __init__(self, vault_path: Path):
        ...
        # Silver Tier: Use categorized subdirectory
        self.needs_action = vault_path / 'Needs_Action' / 'files'
        ...
        self.state_file = self.vault_path.parent / ".watcher_state" / "filesystem_processed.json"
    ...
    def process_file(self, source: Path):
        ...
        try:
            ...
            # Create action file content with Silver Tier category
            content = f'''---
type: file_drop
category: files
...
'''
            ...
            logger.info(f'Created action file: /Needs_Action/files/{action_filename}')
            ...
            if self.state_file.exists():
                ...
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'processed_files' in data:
                        self.processed_files.update(data['processed_files'])
                logger.info(f'Loaded {len(self.processed_files)} previously processed files from state file')
            else:
                logger.info('No state file found, starting fresh')
        except Exception as e:
            logger.warning(f'Could not load state file: {e}')
            self.processed_files.clear()  # Start fresh if there's an error
...
class FileSystemWatcher:
    ...
    - Creates action files in /Needs_Action/files/
    ...
    def __init__(self, vault_path: str):
        ...
        logger.info(f'Output: /Needs_Action/files/')
    ...
    def run(self):
        ...
        try:
            while True:
                # Save processed files state periodically (every 30 seconds)
                self.handler.save_processed_files()
                time.sleep(30)
